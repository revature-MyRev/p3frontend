...
export class FollowerService {
    private baseURL: string = "http://localhost:8090/followers/";
    follower!: IFollower;
    ...
    getFollowedByUserAndProfileId(profileId: number, activeUserId: number) : Observable<IFollower> {
        //Creating headers
        return this.http.get<IFollower>(this.findByFollowedIdAndFollowerId + profileId + "," + activeUserId);
    }
    ...
    addUserToFollowedUsers(follower: IFollower): Observable<any> {
        ...
        console.log("Adding follower with follwerId: " + follower.followerId + " and followedId: " + follower.followedId);
    ...
    updateAFollowerObject(follower: IFollower) : Observable<any> {
        ...
        const body = JSON.stringify(follower); //Creating body
        return this.http.put<any>(this.update + follower.id, body, { headers: headers }) //Sending and returning request
    }
